...
# Targeted vaccination function
def targeted_vaccinate():
    global total_vaccinated
    vaccination_rate = 0.05  # 5% of susceptible individuals vaccinated per step
    eligible_nodes = [
        node for node in graph.nodes()
        if graph.nodes[node]['state'] == 0 or graph.nodes[node]['state'] == 2
    ]
    
    # Select nodes connected to more than 2 infected nodes or with high degree
    high_risk_nodes = [
        node for node in eligible_nodes
        if sum(1 for neighbor in graph.neighbors(node) if graph.nodes[neighbor]['state'] == 1) > 2 or len(list(graph.neighbors(node))) > 10
    ]
    
    num_to_vaccinate = int(vaccination_rate * len(high_risk_nodes))
    vaccinated_nodes = random.sample(high_risk_nodes, min(num_to_vaccinate, len(high_risk_nodes)))
...
def update_for_targeted(frame):
    ax.clear()
    spread_infection()
    targeted_vaccinate()
    color_map = get_color_map()
    nx.draw(graph, node_size=60, node_color=color_map, ax=ax, with_labels=False)
    ax.set_title(f"Time Step {frame + 1}  Targeted Vaccination")
    ...
    if (immune_percentage >= herd_immunity_threshold):
        print(f"Targeted vaccination: Herd immunity achieved at time step {frame+1}!")
...
def plot_time_series():
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("target_plot.png", dpi=300, format="png")

# Create and show animation for targeted vaccination
ani = animation.FuncAnimation(fig, update_for_targeted, frames=time_steps, repeat=False, interval=500)
ani.save('targeted_vaccinate_animation.mp4', fps=2)
plot_time_series()
